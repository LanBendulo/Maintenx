...
(function () {
    ...
    document.getElementById('exportPdfBtn')?.addEventListener('click', async function() {
        const workOrderId = document.getElementById('cost-work-order-id')?.value;
        
        if (!workOrderId) {
            showToast('Unable to export: Work Order ID not found', 'error');
            return;
        }
        ...
        try {
            ...
            btn.disabled = true;
            btn.innerHTML = '<svg class="animate-spin" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10" opacity="0.25"/><path d="M4 12a8 8 0 018-8" opacity="0.75"/></svg> Generating PDF...';
            
            // Fetch PDF from server
            const response = await fetch(`/admin/work-orders/${workOrderId}/export-pdf`, {
                method: 'GET',
                headers: {
                    'RequestVerificationToken': document.querySelector('input[name="__RequestVerificationToken"]').value
                }
            ...
            a.href = url;
            a.download = `WorkOrder_WO-${String(workOrderId).padStart(4, '0')}_${new Date().toISOString().split('T')[0]}.pdf`;
            ...
            const btn = this;
            btn.disabled = false;
            btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" width="15" height="15"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Export PDF';
        }
    });
